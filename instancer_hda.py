...
def execute():
    #nodes = []
    num_assets = hou.pwd().parm("assets").eval() # important
    #print(num_assets)
    ...
    attr_rand_name.parm("distribution").set("discrete") # open the node's parameter interface to find the name of the option
    attr_rand_name.parm("valuetype").set(1)
    attr_rand_name.parm("values").set(num_assets)
    
    # keep as our main loop
